...
def collect_observed_alignments(observed_alns_fp, format="sam"):
...
def compute_accuracy(expected_alns, observed_alns, format="sam", offset=0):
    """For each observed alignment, compute the accuracy score based on the list of
       ...
       format : string, optional
          format of observed alignments (SAM or BLAST)
    ...
    if format == "sam":
        for read_id in observed_alns:
            if read_id not in expected_alns:
                ...
                continue
            obs_contig = observed_alns[read_id][1]
            obs_pos = int(observed_alns[read_id][2])
...
def _main(expected_alns_fp, observed_alns_fp, tool, observed_aln_format, total_reads):
    ...
    observed_alns = collect_observed_alignments(observed_alns_fp=observed_alns_fp, format=observed_aln_format)
